...
export default function PostDetail() {
  ...
  if (loading) return <div className="flex justify-center items-center h-screen text-gray-500">Loading Story...</div>;
    ...
    <div className="max-w-6xl mx-auto p-4 md:p-8 space-y-8">
      ...
      <div className="flex flex-col md:flex-row justify-between items-start gap-4">
        <div className="flex-1 space-y-2">
          ...
          <div className="flex flex-wrap gap-3 items-center text-gray-500 text-sm">
            <span className="bg-green-100 text-green-800 px-3 py-1 rounded-full">{post.category_id?.category_name}</span>
            <span className="flex items-center gap-1"><MapPin size={14} /> {post.province_id?.province_name}</span>
      ...
      <div className="relative flex flex-col md:flex-row gap-4">
        <div className="relative flex-1 h-[600px] rounded-2xl overflow-hidden shadow-lg bg-black">
          {post.images && post.images.length > 0 ? (
            ...
            <div className="w-full h-full flex items-center justify-center text-gray-500 bg-gray-100">No images</div>
          )}
        </div>
      ...
      <div className="space-y-6">
        <div className="flex items-center gap-1">
          ...
          <span className="bg-gray-100 px-3 py-0.5 rounded-full text-sm font-bold">{comments.length}</span>
        </div>
        ...
        <div className="space-y-4">
          {comments.length === 0 ? (
            <p className="text-gray-400 text-center py-8 italic">No comments yet.</p>
          ) : (
            comments.map((comment) => (
              <div key={comment._id} className="flex gap-4 p-4 rounded-2xl bg-white border border-gray-50 hover:border-gray-100 transition shadow-sm">
                ...
                <div className="flex-1">
                  <div className="flex justify-between items-center mb-1">
                    <h4 className="font-bold text-gray-800">{comment.user_id.username}</h4>
                    <span className="text-[10px] text-gray-400 uppercase font-medium">{new Date(comment.created_at).toLocaleDateString()}</span>
                  </div>
