...
// //   console.log("Checking user session:", user); // Check if token exists here
...
// //     try {
// //       const posts = await postService.getPostsByUser(user._id, user.token);
// //       setUserPosts(posts);
// //     } catch (err) {
            ...
            {activeTab === "My Posts" && (
              <div>
                {loadingPosts ? (
                  ...
                  <div className="space-y-6">
                    {userPosts.map((post) => (
                      <div key={post._id} className="border rounded-lg p-4 shadow-sm bg-green">
                        <h2 className="text-lg font-semibold mb-2">{post.title}</h2>
                        <p className="mb-2 whitespace-pre-line">{post.content}</p>
                        {post.location_description && (
                          <p className="text-gray-600 mb-2">📍 {post.location_description}</p>
